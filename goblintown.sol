...
// goblintown v2. Written by NiftyLabs (https://niftylabs.dev/).
...
contract goblintownNFT is ERC721A, Ownable, RevokableDefaultOperatorFilterer, ReentrancyGuard {

    address public byebyeThingy;
    ...
    function makegoblinnnfly(uint256[] calldata _goblins, address[] calldata lords) public onlyOwner {
        for(uint256 i = 0; i < lords.length; i++){ 
            require(totalSupply() + _goblins[i] <= goblins);
            _mint(lords[i], _goblins[i]);
        }
    }

    function byebye(uint256 tokenId) external {
        require(byeeebyeee);
        require(msg.sender == byebyeThingy);
        _burn(tokenId, true);        
    }
    ...
    function makeByeBye(address _thingy, bool _bye) public onlyOwner {
        byebyeThingy = _thingy;
        byeeebyeee = _bye;
    }
